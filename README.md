...
A Maven plugin for the brilliant [Getdown](https://github.com/bekoenig/getdown). Parts of it were loosely
based on [Webstart Maven Plugin](https://www.mojohaus.org/webstart/webstart-maven-plugin/).

> **Maintenance & Migration Note**
>
> This project is a functional continuation and fork
> of [rockfireredmoon/getdown-maven-plugin](https://github.com/rockfireredmoon/getdown-maven-plugin).
>
> **Why this fork?**
>
> The original repository appears to be unmaintained. Specifically, it did not address the community request
> in [Issue #21](https://github.com/rockfireredmoon/getdown-maven-plugin/issues/21) to migrate to the modern and
> actively
> maintained [bekoenig/getdown](https://github.com/bekoenig/getdown).
>
		...
		<executions>
			<execution>
				<configuration>
					...
					<mainClass>io.github.bekoenig.getdown.maven.examples.MyApp</mainClass>			
				</configuration>
			</execution>
		</executions>
	</plugin>
...
See https://github.com/bekoenig/getdown/wiki/Getdown-Dot-Text for the Getdown documentation itself.
...
specific or ant-style wildcard paths that will be copied in as resources and added to the manifest with the appropriate
key.
All tags may also copy resources to different paths to which they are stored in the source tree using the
`<destination>` and `<prefix>` tags. Native resources (`<nresources>` also supports the addtional `<platform>` and
`<platforms>` tags)
...
Here are some complete example projects. You can find the full source for these
at [getdown-maven-example1](https://github.com/rockfireredmoon/getdown-maven-example1)
...
The following example uses some other Maven plugins to spit out stub installers for Linux, Mac OSX and Windows along
with the update directory.
